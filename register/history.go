...
func registerHistory(s *server.MCPServer) {
	...
	s.AddTool(
		mcp.NewTool("get_history_compare",
			...
			mcp.WithString("time_range", mcp.Description("相对时间范围，例如: 7d 15h 30m，可与 start_time/end_time 二选一。注意：若使用 minute 粒度，time_range 最多为 6h。")),
			mcp.WithString("period", mcp.Description("比较粒度: 'hour' / 'day' / 'minute'，默认 'day'。minute 仅在 time_range 或 start/end 范围不超过 6 小时时可用。")),
			mcp.WithNumber("minute_interval", mcp.Description("分钟粒度，仅在 period='minute' 时生效，支持 1、5、15、30，默认 1")),
